...
function initScene() {
    ...
    camera.position.set(0, 1, 5).multiplyScalar(7);
...
function createFloor() {
    ...
    behind.quaternion.setFromAxisAngle(new THREE.Vector3(0, 0, 1), Math.PI * .5);
    scene.add(behind);
    const behindBody = new CANNON.Body({
        type: CANNON.Body.STATIC,
        shape: new CANNON.Plane(),
    });
    behindBody.position.copy(behind.position);
    behindBody.quaternion.copy(behind.quaternion);
    physicsWorld.addBody(behindBody);
...
function throwDice() {
    ...
    diceArray.forEach((d, dIdx) => {
        ...
        d.body.position = new CANNON.Vec3(5, dIdx * 9, 1);
